  ...
  <div class="st-page admin-page">
    <section class="st-hero page-hero">
      <div class="st-hero-content">
        <div class="st-hero-copy">
          <p class="st-hero-eyebrow">活动运营</p>
          <h1 class="st-hero-title">活动管理</h1>
          <p class="st-hero-desc">
            在这里统一处理活动查看、编辑、取消、结项、报名名单和删除等管理操作。
          </p>
        </div>
        <div class="st-hero-actions">
          <span class="st-chip">总数 {{ pagination.total }} 条</span>
          <el-button class="hero-btn" type="primary" @click="router.push('/admin/create')">发布活动</el-button>
          <el-button class="hero-btn" @click="fetchActivities">刷新</el-button>
        </div>
      </div>
    </section>
    ...
    <section class="st-stat-grid">
      <article class="st-stat-card">
        <span class="label">招募中</span>
        <strong class="value">{{ recruitingCount }}</strong>
        <span class="hint">可继续报名</span>
      </article>
      <article class="st-stat-card">
        <span class="label">已结项</span>
        <strong class="value">{{ completedCount }}</strong>
        <span class="hint">服务已完成</span>
      </article>
      <article class="st-stat-card">
        <span class="label">已取消</span>
        <strong class="value">{{ cancelledCount }}</strong>
        <span class="hint">已停止招募</span>
      </article>
    </section>
...
const recruitingCount = computed(() => activities.value.filter(item => item.status === 'RECRUITING').length)
const completedCount = computed(() => activities.value.filter(item => item.status === 'COMPLETED').length)
const cancelledCount = computed(() => activities.value.filter(item => item.status === 'CANCELLED').length)
    ...
    { required: true, message: '请选择活动开始时间', trigger: 'change' },
    {
      validator: (_rule, value, callback) => {
        ...
        if (new Date(value) >= new Date(editForm.endTime)) {
          callback(new Error('活动开始时间需早于结束时间'))
          return
        }
        callback()
      },
    ...
    { required: true, message: '请选择报名截止时间', trigger: 'change' },
    {
      validator: (_rule, value, callback) => {
        ...
        if (new Date(value) <= new Date(editForm.registrationStartTime)) {
          callback(new Error('报名截止需晚于招募开始'))
          return
        }
