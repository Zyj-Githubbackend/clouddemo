    ...
    <div class="preview-grid" :class="{ empty: previewItems.length === 0 }">
      <template v-if="previewItems.length > 0">
        <div v-for="(item, index) in previewItems" :key="item.imageKey || item.imageUrl || index" class="preview-card">
          <img :src="item.imageUrl" alt="公告图片预览" class="preview-image">
          <button
            type="button"
            class="remove-btn"
      ...
      <div v-else class="preview-placeholder">
        <span>Announcement Gallery</span>
        <small>上传公告配图，可多选</small>
      </div>
    </div>
    ...
    <div class="toolbar">
      ...
      >
        <el-button
          ...
          :disabled="disabled || uploading || previewItems.length >= limit"
        >
          {{ previewItems.length > 0 ? '继续上传图片' : '上传公告图片' }}
        </el-button>
      </el-upload>
      <el-button
...
import { buildAnnouncementImageUrl } from '@/utils/announcementImage'
...
const previewItems = computed(() => {
  ...
  return Array.from({ length: maxLength }, (_, index) => ({
    imageKey: keys[index] || '',
    imageUrl: urls[index] || buildAnnouncementImageUrl(keys[index])
