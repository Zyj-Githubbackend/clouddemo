...
@Component
public class FeedbackOutboxPublisher implements EventPublisher {

    private static final Logger log = LoggerFactory.getLogger(FeedbackOutboxPublisher.class);
    private static final int MAX_RETRY = 8;

    private final EventOutboxMapper eventOutboxMapper;
    private final RabbitTemplate rabbitTemplate;

    public FeedbackOutboxPublisher(EventOutboxMapper eventOutboxMapper, RabbitTemplate rabbitTemplate) {
    ...
    public void publishPendingOutbox() {
        ...
        for (EventOutbox outbox : pendingEvents) {
            try {
                ...
                event.setServiceName(System.getenv().getOrDefault("SERVICE_NAME", "feedback-service"));
                ...
                log.warn("failed to publish outbox event stackId={} serviceName={} eventType={} messageId={} outboxId={} retryCount={}",
                        ...
                        System.getenv().getOrDefault("SERVICE_NAME", "feedback-service"),
                        outbox.getEventType(),
                        outbox.getMessageId(),
                        outbox.getId(),
                        nextRetryCount,
