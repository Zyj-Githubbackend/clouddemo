...
SHOW TABLES;
-- 应包含: sys_user, vol_activity, vol_registration，以及视图 v_activity_statistics
SELECT COUNT(*) FROM sys_user;       -- 期望 11（1 管理员 + 10 志愿者）
SELECT COUNT(*) FROM vol_activity;   -- 期望 20
SELECT COUNT(*) FROM vol_registration; -- 期望 54
```

**注意**：默认密码配置为 `123888`，如需修改请编辑各服务的 `application.properties`。种子用户密码均为 `password123`（演示用，生产须改为加密存储）。
...
**验证**：看到类似输出（具体版本以本机 `package.json` 为准）
```
...
你应该看到：
- 🎨 美观的登录页面（渐变背景 + 动态装饰）
- ⚡ **三个**快速登录标签（管理员、学生01、学生02）；数据库另有 `student03`～`student10` 可手动登录
...
| 学生01 | student01 | password123 | 志愿者（种子累计 **11.0** 小时） |
| 学生02 | student02 | password123 | 志愿者（种子累计 **6.0** 小时） |
